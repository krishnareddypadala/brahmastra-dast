...
from server.db import (
    init_db, create_scan, get_scan, list_scans, update_scan_status,
    delete_scan, save_finding, save_event, get_events
)
...
async def lifespan(app: FastAPI):
    """Mark orphaned 'running' scans as failed on startup (server restart recovery)."""
    for s in list_scans():
        if s.get("status") == "running":
            update_scan_status(s["id"], "failed", error="Server restarted — scan interrupted")
    yield

app = FastAPI(title="BRAHMASTRA Dashboard", version="2.0.0", lifespan=lifespan)
...
MODEL_BACKENDS = {
    ...
    "brahmastra":    {"type": "ollama", "url": "http://localhost:11435/api/chat", "model": "brahmastra",  "label": "BRAHMASTRA Fine-tuned (Training...)", "needs_key": False},
        ...
        if ptype == "none":
            ...
            try:
                ...
                async with httpx.AsyncClient(timeout=3.0) as client:
                    ...
                    status[key] = {"online": r.status_code == 200, "label": cfg["label"], "needs_key": False}
            except Exception:
...
async def pause_scan(scan_id: str):
    ...
    if scan_id in _pause_flags:
        ...
        save_event(scan_id, "paused", {"msg": "Scan paused by user"})
...
async def resume_scan(scan_id: str):
    ...
    if scan_id in _pause_flags:
        ...
        save_event(scan_id, "resumed", {"msg": "Scan resumed"})
        for q in _sse_queues.get(scan_id, []):
...
async def stop_scan(scan_id: str):
    ...
    save_event(scan_id, "log", {"msg": "Scan stopped by user", "level": "warn"})
    for q in _sse_queues.get(scan_id, []):
...
async def scan_events(scan_id: str, request: Request):
    ...
    async def event_stream() -> AsyncGenerator[str, None]:
        past = get_events(scan_id, after_id=0)
        for ev in past:
...
async def run_scan_engine(scan_id: str, target: str, req: ScanRequest):
    ...
    def emit_sync(event_type: str, data: dict):
        save_event(scan_id, event_type, data)
        for q in _sse_queues.get(scan_id, []):
    ...
    try:
        emit_sync("log", {"msg": f"BRAHMASTRA v2 engine started — {target}", "level": "info"})
        emit_sync("log", {"msg": f"Profile: {req.scan_profile} | AI: {req.ai_mode} | AuthZ: {req.authz_testing}", "level": "info"})
        ...
        for f in result.findings:
            ...
            save_finding(scan_id, fdict)

        update_scan_status(scan_id, "complete", total_requests=result.total_requests)

    except Exception as e:
        import traceback
        tb = traceback.format_exc()
        update_scan_status(scan_id, "failed", error=str(e))
        emit_sync("error", {"msg": str(e), "trace": tb})
        emit_sync("done", {"error": str(e)})
    finally:
...
async def run_scan(scan_id: str, target: str, req: ScanRequest):
    ...
    try:
        emit("log", {"msg": f"BRAHMASTRA scan started — {target}", "level": "info"})
        emit("log", {"msg": f"Profile: {req.scan_profile} | Auth: {req.auth_type}", "level": "info"})
        ...
        if req.username and req.password and req.auth_type == "form":
            emit("log", {"msg": f"Performing form login as {req.username}...", "level": "info"})
            login_cookies = await do_form_login(target, req.username, req.password)
            if login_cookies:
                cookies.update(login_cookies)
                emit("log", {"msg": "Login successful — session cookies captured", "level": "success"})
            else:
                emit("log", {"msg": "Login failed — continuing unauthenticated", "level": "warn"})

        emit("log", {"msg": "Garudastra — crawling target...", "level": "info"})
        test_points = await crawl(target, headers, cookies)
        emit("log", {"msg": f"Found {len(test_points)} test points", "level": "info"})

        if not test_points:
            emit("log", {"msg": "No test points found — using defaults", "level": "warn"})
            test_points = [
        ...
        for tp in test_points:
            ...
            if pause_flag and not pause_flag.is_set():
                emit("log", {"msg": "⏸ Scan paused — waiting for resume...", "level": "warn"})
                await pause_flag.wait()
                if _stop_flags.get(scan_id):
                    stopped = True
                    break
                emit("log", {"msg": "▶ Scan resumed", "level": "success"})
            ...
            emit("probe", {"path": path, "param": param, "method": method,
            ...
            for vuln_type, payloads in vuln_types:
                if _stop_flags.get(scan_id):
                    ...
                    break
                if pause_flag and not pause_flag.is_set():
                    emit("log", {"msg": "⏸ Paused mid-test — waiting...", "level": "warn"})
                    await pause_flag.wait()
                    if _stop_flags.get(scan_id):
                        stopped = True
                        break
                    emit("log", {"msg": "▶ Resumed", "level": "success"})
                ...
                for payload in payloads:
                    ...
                    if finding:
                        fid = save_finding(scan_id, finding)
                        finding["id"] = fid
                        findings_count += 1
        ...
        if stopped:
            emit("log", {"msg": f"Scan stopped — {findings_count} findings, {total_requests} requests", "level": "warn"})
        else:
            emit("log", {"msg": f"Scan complete — {findings_count} findings, {total_requests} requests", "level": "success"})

        update_scan_status(scan_id, "complete", total_requests=total_requests)
        emit("done", {"findings": findings_count, "requests": total_requests, "stopped": stopped})

    except Exception as e:
        import traceback
        tb = traceback.format_exc()
        update_scan_status(scan_id, "failed", error=str(e))
        emit("error", {"msg": str(e), "trace": tb})
        emit("done", {"error": str(e)})
...
async def test_payload(
    ...
    try:
        ...
        emit("log", {"msg": f"Request error: {type(e).__name__}: {e or repr(e)}", "level": "warn"})
        return None
